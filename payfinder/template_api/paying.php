...
$myUrlDir = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http")
."://$_SERVER[HTTP_HOST]"
...
$params = array(
  ...
  "url_notify" => "$myUrlDir/response_collector.php?notify=1", // The URL for notifying your system with the transaction result. Must include http:// or https://. This parameter can be configured in Hosted Page V.2 under the Management section. If specified in the request, overrides the configured value.
  "url_redirect" => "$myUrlDir/response_collector.php?redirect=1", // The URL to which the buyer’s browser is redirected to after completing the payment Must include http:// or https://. This parameter can be configured in under the Management section. If specified in the request, overrides the configured value.
...
<?php } elseif (array_key_exists('popup', $_GET)) {?>
  <script>
    window.setTimeout(
      () => window.open(
        '<?=$url?>',
